...
const HTML_FOLDER = "/app/wikitech.wikimedia.org";  // folder with your HTML files
...
const EXCLUDE_DIRS = ["Shell_Request", "Server_Admin_Log", "Category", "Deployments"];
...
function findHtml(folder) {
    ...
    for (var i = 0; i < htmls.length; i++) {
        var filename = htmls[i];
        if (filename.startsWith("Template_")) {continue;}
        if (filename.startsWith("Obsolete")) {continue;}
        if (filename.startsWith("Talk_")) {continue;}
        if (filename.startsWith("File_")) {continue;}
        if (filename.startsWith("User_talk_")) {continue;}
        if (filename.startsWith("Category_")) {continue;}
        realhtmls.push(filename);
    }
    return realhtmls;
};
...
function readHtml(root, file, fileId) {
    ...
    // Cheerio will just dump a bunch of style nonsense into our summary
    $('style').remove();
    var body = $(".mw-body-content").text();
